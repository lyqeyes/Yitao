...
namespace YiTao.Web.Areas.Watch.Controllers
{
    public class YitaoController : BaseController
    {
        ...
        [NoLogin]
        public ActionResult AreaInfo(int id)
        {
            var area = db.Areas.FirstOrDefault(a => a.Id == id);
            HttpCookie hc = new HttpCookie("Area");
            hc.Values["Name"] = HttpUtility.UrlEncode(area.Name);
            hc.Values["Id"] = area.Id.ToString();
        ...
        public ActionResult Productlist(int id)
        {
            ...
            else
            {
                ...
                if (logininfo == null)
                {
                    ...
                    data = (from a in db.ShangPin2Areas
                             where a.Type==(int)EnumAreaType.ZhuantiItem 
                             &&a.AreaId == CityContext.Current.CityId
                             join b in db.ZhuanTiItems
                             on a.ForId equals b.ZhuanTiItemId
                             where b.ZhuanTiId == temp.ZhuanTiId
                ...
                else
                {
                    ...
                    data = (from a in db.ShangPin2Areas
                            ...
                            on a.ForId equals z.ZhuanTiItemId
                            where z.ZhuanTiId == temp.ZhuanTiId
                            select new Statistic<ZhuanTiItem>
                            {
                                data = z,
                                IsCollected = db.Collections.Where(b => b.Type ==
                                    (int)EnumCollectionType.ZhuantiItem && b.AccountId
                                    == account.AccountId &&
                                    b.ShangPinId == z.ZhuanTiItemId).Count()
                            }).ToList();
                    //data = (from z in db.ZhuanTiItems
                    //        where z.ZhuanTiId == temp.ZhuanTiId
                    //        select new Statistic<ZhuanTiItem>
        ...
        public ActionResult ThreeLei(int id)
        {
            ...
            {
                var query = from a in db.ShangPin2Areas
                            where a.AreaId == CityContext.Current.CityId &&
                            a.Type == (int)EnumAreaType.NormalShangpin
                            join b in db.ShangPins
                            on a.ForId equals b.ShangPinId
                            where b.XiaoLeiId == item.ThreeLeiId
                            select b;
                //allShangpin.AddRange(db.ShangPins.Where(s => s.XiaoLeiId == 
                //    item.ThreeLeiId));
                allShangpin.AddRange(query.ToList());
            }
        ...
        {
            List<OtherUrl> urlList = 
                db.OtherUrls.OrderByDescending(a => a.Weight).Take(3).ToList();
            return View(urlList);
        }
        [NoLogin]
        ...
        public ActionResult Juzekou()
        {
            ...
            if (logininfo == null)
            {
                ...
                //        }).Take(3).ToList();
                data = (from a in db.ShangPin2Areas
                        where a.Type == (int)EnumAreaType.Juzhekou
                        && a.AreaId == CityContext.Current.CityId
                        join z in db.JuZheKouItems
                        on a.ForId equals z.JuZheKouItemId
                        orderby z.JuZheKouItemId descending
                        select new JuzekouStatistic
                        {
                            juzhekou = z,
                            //处理每一个聚折扣商品的是否被当前用户收藏
            ...
            else
            {
                ...
                data = (from a in db.ShangPin2Areas
                        ...
                        select new JuzekouStatistic
                        {
                            juzhekou = z,
                            //处理每一个聚折扣商品的是否被当前用户收藏
                            IsCollected = db.Collections.Where(b =>
                                b.Type == (int)EnumCollectionType.Juzhekou &&
                                b.AccountId == account.AccountId && b.ShangPinId
                                == z.JuZheKouItemId).Count()
                        }).Take(3).ToList();
                //data = (from a in db.JuZheKouItems
                //        orderby a.JuZheKouItemId descending
                //        select new JuzekouStatistic
